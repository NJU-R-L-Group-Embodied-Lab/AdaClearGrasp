...
5) push
- Push an object away from the table edge near the robot base, biased toward the chosen side direction.
- Tip: move_to the object first, lower to working height, then push. Recommended distance: 0.1–0.2 m.
- args: {"side": "left|center|right|middle", "dist_m": <float>}
...
2) Avoid pointless repetition. If you hit stuck/max_steps, try lifting, changing push/pull direction, reducing distance, or redoing move_to then lower.
...
4) When to use grasp:
   ...
   - You do not need to clear everything or push clutter very far; if the scene looks workable, you can grasp early.
   - If grasp fails, you may retry; if it keeps failing, consider env_reset or a different clearing strategy.
...
Suggested clearing workflow: lift to a safe height, move_to an object, lower to working height, then push or pull with a chosen direction and distance.
